...
import it.siliconsquare.model.component.PowerSupply;
import org.springframework.beans.factory.annotation.Autowired;

import java.util.ArrayList;
import java.util.List;

public class PowerSupplyDAOImpl implements ComponentDAO<PowerSupply> {

    @Autowired
    private List<PowerSupply> powerSupplyList;

    private static final String tablename = "power_supply";
    private static final String pk = "id_power_supply";
    ...
    public PowerSupplyDAOImpl() {
        powerSupplyList = new ArrayList<>();
    }

    @Override
    public List<PowerSupply> getAll() {
        return Q2ObjList.fromSelect(PowerSupply.class, sql);
    }

    @Override
    public List<PowerSupply> getAllPublished() {
        String clause = " AND state.id_state = 1";
        return Q2ObjList.fromSelect(PowerSupply.class, sql + clause);
    }

    @Override
    public List<PowerSupply> getAllUnpublished() {
        String clause = " AND state.id_state != 1";
        return Q2ObjList.fromSelect(PowerSupply.class, sql + clause);
    }
    ...
    public PowerSupply getById(int id) {
        ...
        return Q2Obj.fromSelect(PowerSupply.class, sql + clause, id);
    }

    @Override
    public PowerSupply getPublishedById(int id) {
        String clause = " AND state.id_state = 1 AND " + pk + " = ?";
        return Q2Obj.fromSelect(PowerSupply.class, sql + clause, id);
    }

    @Override
    public boolean save(PowerSupply c) {
        c = Q2Obj.insert(c);
    ...
    @Override
    public boolean update(PowerSupply c) {
        PowerSupply newC = Q2Obj.update(c);
        newC.setState(c.getState());
        return newC != null && newC.getId() != 0;
    }

    @Override
    public int setState(PowerSupply c, int id) {
        return Q2Sql.executeUpdate("UPDATE " + tablename + " SET id_state = ? WHERE " + pk + " = ?", id, c.getId());
    }

    @Override
    public List<PowerSupply> getFields() {
        String clause = " AND " + pk + " = 1";
        return Q2ObjList.fromSelect(PowerSupply.class, sql_not_visible + clause);
    }
}
