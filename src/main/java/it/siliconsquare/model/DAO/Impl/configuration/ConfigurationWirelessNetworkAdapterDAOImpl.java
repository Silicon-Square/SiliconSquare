...
public class ConfigurationWirelessNetworkAdapterDAOImpl implements ConfigurationComponentDAO<WirelessNetworkAdapter> {
    @Override
    public WirelessNetworkAdapter getById(Configuration config, int id) {
        ...
        WirelessNetworkAdapter c = Q2Obj.fromSelect(WirelessNetworkAdapter.class, selectQuery, config.getIdConfiguration(),
                id);
        return c;
    }

    @Override
    public boolean save(Configuration config, WirelessNetworkAdapter configurationComponent) {
        ...
        String message = "Attemped to save wireless network adapter with id " + idComponent + " into configuration "
                + config.getIdConfiguration();
        if (affectedRows > 0)
            message = "Saved WirelessNetworkAdapter [" + idComponent + "]  into configuration ["
                    + config.getIdConfiguration()
                    + "]";
    ...
    public boolean delete(Configuration config, int id) {
        ...
        String message = "Attemped to delete wireless network adapter with id " + id + " from configuration "
                + config.getIdConfiguration();
        if (affectedRows > 0)
            message = "Deleted WirelessNetworkAdapter [" + id + "]  from configuration [" + config.getIdConfiguration()
                    + "]";
        Logger.getInstance().captureMessage(message);
        return affectedRows > 0;
    }
    ...
    @Override
    public boolean update(Configuration config, WirelessNetworkAdapter oldComponent, WirelessNetworkAdapter newComponent) {
        ...
        int affectedRows = Q2Sql.executeUpdate(updateQuery, newComponent.getId(), config.getIdConfiguration(),
                oldComponent.getId());
        String message = "Attemped to update wireless network adapter with id " + oldComponent.getId()
                + " from configuration "
                + config.getIdConfiguration();
        if (affectedRows > 0)
            message = "Updated WirelessNetworkAdapter [" + oldComponent.getId() + " --> " + newComponent.getId()
                    + "]  from configuration ["
                    + config.getIdConfiguration() + "]";
        Logger.getInstance().captureMessage(message);
        return affectedRows > 0;
